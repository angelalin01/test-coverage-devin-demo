...
def _reorder_packets(buffer: List[TelemetryPacket]) -> List[TelemetryPacket]:
    """
    Reorder packets by sequence number if present, preserving unsequenced packets.
    
    Packets with sequence numbers are sorted first, followed by unsequenced packets
    in their original order to avoid data loss from legacy systems.
    """
...
class TelemetryReceiver:
    ...
    async def receive_packet_async(self, packet: TelemetryPacket, retry_count: int = 0) -> bool:
        ...
        Increments error_count when exceptions occur for monitoring visibility.
        """
        try:
